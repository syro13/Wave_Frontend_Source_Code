...
public class HouseBreakdownBottomSheet extends BottomSheetDialogFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.bottom_sheet_smart_breakdown, container, false);
    ...
    private void initTaskTypeChips() {
        String[] taskTypes = {"Cleaning", "Cooking", "Laundry", "Gardening", "Maintenance", "Grocery Shopping"};
    ...
    private void loadTasksIntoSpinner() {
        ...
        FirebaseFirestore.getInstance()
                .collection("users").document(uid).collection("housetasks")
    ...
    private void requestBreakdown(String taskTypes) {
        ...
        String specificTaskPrompt = getHouseTaskSpecificPrompt(taskTypes);

        String prompt = String.format(
                "You are an expert home chore planner who loves to add fun and motivational emojis.\n\n" +
                        "TASK: '%s'\n" +
                        "TASK TYPE: '%s'\n" +
                        "DETAILS: '%s'\n" +
                        "DEADLINE: '%s'\n" +
                        ...
                        "Create a smart breakdown with a list of subtasks for house tasks. Use emojis where appropriate to improve readability." +
                        ...
                        "Include an emoji at the start of each subtask name for visual appeal (e.g., 🧹 for cleaning, 🍳 for cooking).\n\n" +
                ...
                .enqueue(new Callback<SchoolTasksFragment.AIResponse>() {
                    ...
                    public void onResponse(Call<SchoolTasksFragment.AIResponse> call, Response<SchoolTasksFragment.AIResponse> response) {
                        ...
                        if (response.isSuccessful() && response.body() != null && listener != null) {
                            ...
                            if (validateTimeAllocation(aiResponse, availableHours)) {
                                listener.onBreakdownReady(aiResponse);
                                dismiss();

                                saveBreakdownToHistory(task, taskTypes, aiResponse);
                            } else {
    ...
    private String getHouseTaskSpecificPrompt(String taskTypes) {
        Map<String, String> taskPrompts = new HashMap<>();
        taskPrompts.put("Cleaning", "• Break down cleaning into dusting, vacuuming, and mopping.\n• Focus on one room at a time for efficiency.");
        taskPrompts.put("Cooking", "• Include meal planning, ingredient prep, cooking, and plating phases.\n• Suggest meal prep techniques to save time.");
        taskPrompts.put("Laundry", "• Divide into sorting, washing, drying, and folding steps.\n• Include tips for stain removal and energy saving.");
        taskPrompts.put("Gardening", "• Split tasks into weeding, watering, and pruning.\n• Recommend seasonal care tips.");
        taskPrompts.put("Maintenance", "• Include inspection, minor repairs, and scheduling professional services.\n• Prioritize urgent fixes.");
        taskPrompts.put("Grocery Shopping", "• Organize the shopping list by store sections.\n• Suggest planning routes and checking for deals.");
    ...
    private void saveBreakdownToHistory(String taskName, String taskType, String breakdown) {
        ...
        FirebaseFirestore.getInstance()
                ...
                .collection("houseBreakdownHistory")
                .add(historyItem);
    }
    ...
    private void requestBreakdownWithStricterConstraints(String task, String taskTypes, String time,
        ...
        String strictPrompt = String.format(
                "CRITICAL: Create a time-strict breakdown with the following constraints:\n\n" +
                        ...
                        "• Include a 'Goal', 'How' instructions, and a motivational tip, Use emojis where appropriate to improve readability.\n\n" +
                        "FORMAT:\n" +
